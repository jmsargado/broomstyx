...
PlaneStrain_Fe_CrackTip::CellNumericsStatus::CellNumericsStatus( int nGaussPts )
{
    _nGaussPts = nGaussPts;
    _gp.assign( _nGaussPts, EvalPoint() );
    
    for ( int i = 0; i < _nGaussPts; i++ )
        _gp[ i ].numericsStatus = new EvalPtNumericsStatus();
}

PlaneStrain_Fe_CrackTip::CellNumericsStatus::~CellNumericsStatus()
{
    for ( int i = 0; i < _nGaussPts; i++ )
...
PlaneStrain_Fe_CrackTip::EvalPtNumericsStatus::EvalPtNumericsStatus()
...
PlaneStrain_Fe_CrackTip::EvalPtNumericsStatus::~EvalPtNumericsStatus() = default;
...
void PlaneStrain_Fe_CrackTip::initializeNumericsAt( Cell* targetCell ) 
{
    ...
    targetCell->numericsStatus = new CellNumericsStatus( nGaussPts );
    auto cns = this->getNumericsStatusAt( targetCell );
    for ( int i = 0; i < nGaussPts; i++ )
    {
...
// ---------------------------------------------------------------------------
PlaneStrain_Fe_CrackTip::CellNumericsStatus*
...
// ---------------------------------------------------------------------------
PlaneStrain_Fe_CrackTip::EvalPtNumericsStatus*
PlaneStrain_Fe_CrackTip::getNumericsStatusAt( EvalPoint& gp )
{
    auto gpns = dynamic_cast< EvalPtNumericsStatus* >( gp.numericsStatus );
    if ( !gpns )
...
RealVector PlaneStrain_Fe_CrackTip::giveBasisFunctionsAt( const RealVector& natCoor )
{
    ...
    RealVector psi_c( { psi( 0 ) + psi( 3 ) + psi( 7 ),
                        psi( 1 ),
                        psi( 2 ),
                        psi( 4 ),
                        psi( 5 ),
...
std::vector<RealVector> PlaneStrain_Fe_CrackTip::giveBasisFunctionDerivativesAt( const RealVector& natCoor )
{
    ...
    dpsi_c[ 0 ] = { dpsi[ 0 ]( 0 ) + dpsi[ 0 ]( 3 ) + dpsi[ 0 ]( 7 ),
                    dpsi[ 0 ]( 1 ),
                    dpsi[ 0 ]( 2 ),
                    dpsi[ 0 ]( 4 ),
                    dpsi[ 0 ]( 5 ),
    ...
    dpsi_c[ 1 ] = { dpsi[ 1 ]( 0 ) + dpsi[ 1 ]( 3 ) + dpsi[ 1 ]( 7 ),
                    dpsi[ 1 ]( 1 ),
                    dpsi[ 1 ]( 2 ),
                    dpsi[ 1 ]( 4 ),
                    dpsi[ 1 ]( 5 ),
...
#pragma GCC ivdep
    for ( int i = 0; i < 12; i++ )
        u( i ) = DofManager::giveValueOfPrimaryVariableAt( dof[ i ], valType );
