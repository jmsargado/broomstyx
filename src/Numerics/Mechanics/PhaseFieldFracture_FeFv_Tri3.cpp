...
void PhaseFieldFracture_FeFv_Tri3::finalizeDataAt( Cell* targetCell, const TimeData& time )
{
    ...
    cns->_phi = DofManager::giveValueOfPrimaryVariableAt( dof1, converged_value );
    ...
    for ( int i = 0; i < 3; i++ )
    {
        ...
        if ( cns->_hasPhsFldPrescribedOnFace[ i ] )
        {
            ...
            double d1 = giveDistanceToMidpointOf( face[ i ], cellCoor );
        ...
        else
        {
            ...
            double phi2 = DofManager::giveValueOfPrimaryVariableAt( dof2, converged_value );
            
            d_pf_n = cns->_transmissibility[ i ] * ( phi2 - cns->_phi ) / ( length * _l * _l );
        }
    ...
    for ( int i = 0; i < 3; i++ )
    {
        ...
        d_pf = d_pf + cns->_valueOnFace[ i ] * length / ( 2. * cns->_area ) * ( cellCoor - vtxNodeCoor );
    }
    ...
    if ( cns->_phi < CRACKLENGTH_CUTOFF )
        cns->_crackDensity = ( _l * cns->_gradPhi.dot( cns->_gradPhi ) + cns->_phi * cns->_phi / _l ) /2.;
    else
        cns->_crackDensity = ( _l * cns->_gradPhi.dot( cns->_gradPhi ) ) / 2.;
    ...
    RealVector conState2( { cns->_phi,
                            -_l * d_degFcn * drvForce,
                            time.giveCurrentTime(),
                            time.giveTargetTime() } );

    cns->_Gbulk = -_l * d_degFcn * drvForce;

    // Update history field
    if ( cns->_phi > _phiIrrev && drvForce > cns->_histFld )
        cns->_histFld = drvForce;

    material[ 2 ]->updateStatusFrom( conState2, cns->_materialStatus[ 2 ], "FinalizeHistoryField" );
    RealMatrix Gc = material[ 2 ]->giveModulusFrom( conState2, cns->_materialStatus[ 2 ] );
    cns->_Gc = Gc( 0,0 );
    cns->_surfEgy = Gc( 0,0 ) * cns->_crackDensity;
}
// ----------------------------------------------------------------------------
...
PhaseFieldFracture_FeFv_Tri3::giveStaticCoefficientMatrixAt( Cell*           targetCell
                                                           , int             stage
                                                           , int             subsys
                                                           , const TimeData& time )
{
    ...
    if ( stage == _stage[ 0 ] )
    {
        ...
        if ( subsys == _subsystem[ 1 ] || subsys == UNASSIGNED )
        {
            ...
            double coef = cns->_area * ( _l * dd_degFcn * drvForce / cns->_Gc + 1. );
            ...
            for ( int i = 0; i < 3; i++ )
            {
                ...
                if ( cns->_hasPhsFldPrescribedOnFace[ i ] )
                {
                    ...
                    coefVal( startIdx ) += length * _l * _l / d1;
                }
                else if ( !cns->_hasPhsFldGradientPrescribedOnFace[ i ] )
                {
...
PhaseFieldFracture_FeFv_Tri3::giveStaticLeftHandSideAt( Cell*           targetCell
                                                      , int             stage
                                                      , int             subsys
                                                      , const TimeData& time )
{
    ...
    if ( stage == _stage[ 0 ] )
    {
        ...
        int vecLength = 11; // Default output lengths for unassigned subsystems
        ...
        Dof* dof_phi = analysisModel().domainManager().giveCellDof( _cellDof[ 0 ], targetCell );
        cns->_phi = DofManager::giveValueOfPrimaryVariableAt( dof_phi, current_value );
        ...
        if ( subsys == _subsystem[ 1 ] || subsys == UNASSIGNED )
        {
            ...
            for ( int i = 0; i < 3; i++ )
            {
                if ( cns->_hasPhsFldPrescribedOnFace[ i ] ) // Cell face is a boundary where phase-field is specified
                {
                    ...
                    normFlux( i ) = length * _l * _l * cns->_phi / d1;
                }
                else if ( !cns->_hasPhsFldGradientPrescribedOnFace[ i ] )
                {
                    // Phase-field at neighbor cell
                    Dof* dof_phi2 = analysisModel().domainManager().giveCellDof( _cellDof[ 0 ], neighbor[ i ] );
                    double phi2 = DofManager::giveValueOfPrimaryVariableAt( dof_phi2, current_value );
            ...
            rowDof[ offset + 3 ] = dof_phi;
            rowDof[ offset + 4 ] = dof_phi;
            
            // Calculate FmatPhi
            lhs( offset ) = cns->_area * cns->_phi;
            lhs( offset + 1 ) = normFlux( 0 );
            lhs( offset + 2 ) = normFlux( 1 );
            lhs( offset + 3 ) = normFlux( 2 );
            lhs( offset + 4 ) = cns->_area * _l * d_degFcn * drvForce / cns->_Gc;
        }
    }
...
// ----------------------------------------------------------------------------
std::tuple< std::vector<Dof*>, RealVector >
PhaseFieldFracture_FeFv_Tri3::giveStaticRightHandSideAt( Cell*                    targetCell
                                                       , int                      stage
                                                       , int                      subsys
                ...
                if ( this == domCellNumerics )
                {
                    ...
                    for ( int i = 0; i < 3; i++ )
                    {
                             ...
                             ( bndCellNode[ 1 ] == face[ i ][ 0 ] && bndCellNode[ 0 ] == face[ i ][ 1 ] ) )
                        {
                            ...
                            double length = giveLengthOf( face[ i ] );
                            double d = giveDistanceToMidpointOf( face[ i ], cellCoor[ 0 ] );
                            rhs( 0 ) = length * _l * _l * bcVal / d;
                        }
                    }
                }
            }
...
// ----------------------------------------------------------------------------
std::tuple< std::vector<Dof*>, std::vector<Dof*>, RealVector >
PhaseFieldFracture_FeFv_Tri3::giveTransientCoefficientMatrixAt( Cell*           targetCell
                                                              , int             stage
                                                              , int             subsys
                                                              , const TimeData& time )
{
    ...
    if ( stage == _stage[0] && (subsys == _subsystem[0] || subsys == UNASSIGNED ) )
    {
        ...
        RealMatrix Mmat;
        Mmat = cns->_area * _dampingCoef * _massMatrix;

        rowDof.assign( 18, nullptr );
        colDof.assign( 18, nullptr );
        coefVal.init( 18 );

        rowDof[ 0 ] = nodalDof[ 0 ]; colDof[ 0 ] = nodalDof[ 0 ], coefVal( 0 ) = Mmat( 0, 0 );
        rowDof[ 1 ] = nodalDof[ 0 ]; colDof[ 1 ] = nodalDof[ 2 ], coefVal( 1 ) = Mmat( 0, 1 );
        rowDof[ 2 ] = nodalDof[ 0 ]; colDof[ 2 ] = nodalDof[ 4 ], coefVal( 2 ) = Mmat( 0, 2 );
        rowDof[ 3 ] = nodalDof[ 2 ]; colDof[ 3 ] = nodalDof[ 0 ], coefVal( 3 ) = Mmat( 1, 0 );
        rowDof[ 4 ] = nodalDof[ 2 ]; colDof[ 4 ] = nodalDof[ 2 ], coefVal( 4 ) = Mmat( 1, 1 );
        rowDof[ 5 ] = nodalDof[ 2 ]; colDof[ 5 ] = nodalDof[ 4 ], coefVal( 5 ) = Mmat( 1, 2 );
        rowDof[ 6 ] = nodalDof[ 4 ]; colDof[ 6 ] = nodalDof[ 0 ], coefVal( 6 ) = Mmat( 2, 0 );
        rowDof[ 7 ] = nodalDof[ 4 ]; colDof[ 7 ] = nodalDof[ 2 ], coefVal( 7 ) = Mmat( 2, 1 );
        rowDof[ 8 ] = nodalDof[ 4 ]; colDof[ 8 ] = nodalDof[ 4 ], coefVal( 8 ) = Mmat( 2, 2 );

        rowDof[ 9 ]  = nodalDof[ 1 ]; colDof[ 9 ]  = nodalDof[ 1 ], coefVal( 9 ) = Mmat( 0, 0 );
        rowDof[ 10 ] = nodalDof[ 1 ]; colDof[ 10 ] = nodalDof[ 3 ], coefVal( 10) = Mmat( 0, 1 );
        rowDof[ 11 ] = nodalDof[ 1 ]; colDof[ 11 ] = nodalDof[ 5 ], coefVal( 11 ) = Mmat( 0, 2 );
        rowDof[ 12 ] = nodalDof[ 3 ]; colDof[ 12 ] = nodalDof[ 1 ], coefVal( 12 ) = Mmat( 1, 0 );
        rowDof[ 13 ] = nodalDof[ 3 ]; colDof[ 13 ] = nodalDof[ 3 ], coefVal( 13 ) = Mmat( 1, 1 );
        rowDof[ 14 ] = nodalDof[ 3 ]; colDof[ 14 ] = nodalDof[ 5 ], coefVal( 14 ) = Mmat( 1, 2 );
        rowDof[ 15 ] = nodalDof[ 5 ]; colDof[ 15 ] = nodalDof[ 1 ], coefVal( 15 ) = Mmat( 2, 0 );
        rowDof[ 16 ] = nodalDof[ 5 ]; colDof[ 16 ] = nodalDof[ 3 ], coefVal( 16 ) = Mmat( 2, 1 );
        rowDof[ 17 ] = nodalDof[ 5 ]; colDof[ 17 ] = nodalDof[ 5 ], coefVal( 17 ) = Mmat( 2, 2 );
    }
...
// ----------------------------------------------------------------------------
std::tuple< std::vector<Dof*>, RealVector >
PhaseFieldFracture_FeFv_Tri3::giveTransientLeftHandSideAt( Cell*           targetCell
                                                         , int             stage
                                                         , int             subsys
                                                         , const TimeData& time
                                                         , ValueType       valType )
{
    ...
    {
        std::vector<Dof*> nodalDof = this->giveNodalDofsAt( targetCell );
        RealVector uVec = this->giveLocalDisplacementsAt( nodalDof, valType );

        auto cns = this->getNumericsStatusAt( targetCell );

        // Rearrange vectors to enable use of smaller mass matrix
        RealVector uVecX( { uVec( 0 ), uVec( 2 ), uVec( 4 ) } );
        RealVector uVecY( { uVec( 1 ), uVec( 3 ), uVec( 5 ) } );
        RealVector Mmat_uVecX, Mmat_uVecY;
        Mmat_uVecX = cns->_area * _dampingCoef * _massMatrix * uVecX;
        Mmat_uVecY = cns->_area * _dampingCoef * _massMatrix * uVecY;
...
// ----------------------------------------------------------------------------
void PhaseFieldFracture_FeFv_Tri3::imposeConstraintAt( Cell*                    targetCell
                                                     , int                      stg
                                                     , const BoundaryCondition& bndCond
                                                     , const TimeData&          time )
{
    ...
    if ( bndCond.conditionType() == "PhaseField" || bndCond.conditionType() == "PhaseFieldGradient" )
    {
        ...
        for ( Cell* curDomCell : domCell )
        {
            ...
            if ( this == domCellNumerics )
            {
                ...
                if ( bndCellNode.size() != 2 )
                    throw std::runtime_error( "Numerics type '" + _name + "' only accepts boundary cells with exactly two nodes for phase-field BCs!" );
...
#pragma omp parallel for reduction (+:crackCellCount)
    ...
    for ( int i = 0; i < nCells; i++ )
    {
        ...
        if ( numerics == this )
        {
            ...
            double phi = DofManager::giveValueOfPrimaryVariableAt( phiDof, current_value );
...
void PhaseFieldFracture_FeFv_Tri3::readAdditionalDataFrom( FILE* fp )
{
    ...
    verifyKeyword( fp, "EnableOverdamping", _name );
    std::string choice;
    choice = getStringInputFrom( fp, "Failed to read overdamping option from input file!", _name );
    if ( choice == "Yes" )
        _dampingCoef = getRealInputFrom( fp, "Failed to read damping coefficient from input file!", _name );
    else if ( choice != "No" )
        throw std::runtime_error( "Error: Unrecognized option '" + choice + "' for energy conservation encountered!\n" );
}
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
double PhaseFieldFracture_FeFv_Tri3::giveTransmissibilityCoefficientAt( std::vector<Node*>& face
                                                                      , Cell*               targetCell
                                                                      , Cell*               neighborCell )
{
    // A. Target cell
