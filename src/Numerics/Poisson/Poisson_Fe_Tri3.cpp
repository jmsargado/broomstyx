...
// Numerics status
NS_Poisson_Fe_Tri3::NS_Poisson_Fe_Tri3()
    : _gradU( RealVector(2) )
    , _Jdet( 0. )
{}

NS_Poisson_Fe_Tri3::~NS_Poisson_Fe_Tri3() = default;
...
// Destructor
Poisson_Fe_Tri3::~Poisson_Fe_Tri3() = default;
...
{
    std::vector<Node*> node = DomainManager::giveNodesOf( targetCell );
    ...
    for ( int i : { 0, 1, 2 } )
    {
        RealVector nodeCoor = DomainManager::giveCoordinatesOf( node[ i ] );
...
Poisson_Fe_Tri3::giveFieldOutputAt( Cell* targetCell, const std::string& fieldTag )
{
    ...
    std::vector<Material*> material = Numerics::giveMaterialSetFor( targetCell );
...
{   
    ...
    if ( subsys == _subsystem[0] || subsys == UNASSIGNED )
    {
        ...
        if ( cndType == "NormalGradient" )
        {
            ...
            coor0 = DomainManager::giveCoordinatesOf( node[ 0 ] );
            coor1 = DomainManager::giveCoordinatesOf( node[ 1 ] );
            dx = coor1 - coor0;
            ...
            rowDof[ 0 ] = DomainManager::giveNodalDof( dofNum, node[ 0 ] );
            rowDof[ 1 ] = DomainManager::giveNodalDof( dofNum, node[ 1 ] );
        }
    }
...
{                    
    ...
    if ( bndCond.conditionType() == "NodalConstraint" )
    {
        ...
        for ( auto& j : node )
        {
            Dof* targetDof = DomainManager::giveNodalDof( dofNum, j );
            RealVector coor = DomainManager::giveCoordinatesOf( j );
...
{
    targetCell->numericsStatus[ _stage ] = new NS_Poisson_Fe_Tri3();
    auto cns = this->getNumericsStatusAt( targetCell );
...
void Poisson_Fe_Tri3::setDofStagesAt( Cell* targetCell )
{
    ...
    for ( auto& i : node )
    {
        Dof* dof = DomainManager::giveNodalDof( _nodalDof[0], i );
        DofManager::setStageFor( dof, _stage );
    }
}

// Private methods
// ---------------------------------------------------------------------------
NS_Poisson_Fe_Tri3* Poisson_Fe_Tri3::getNumericsStatusAt( Cell* targetCell )
{
    auto cns = dynamic_cast<NS_Poisson_Fe_Tri3*>( targetCell->numericsStatus[ _stage ] );
    if ( !cns )
...
std::vector<Dof*> Poisson_Fe_Tri3::giveNodalDofsAt( Cell* targetCell )
{
    ...
    dof[ 0 ] = DomainManager::giveNodalDof( _nodalDof[ 0 ], node[ 0 ] );
    dof[ 1 ] = DomainManager::giveNodalDof( _nodalDof[ 0 ], node[ 1 ] );
    dof[ 2 ] = DomainManager::giveNodalDof( _nodalDof[ 0 ], node[ 2 ] );
