...
void GmshReader::readMeshFile( std::string filename )
{
    ...
    // Mesh format
    verifyDeclaration( fp, "$MeshFormat", _name );
    double meshFormat = getRealInputFrom( fp, "Failed to read mesh format from mesh file!", _name );
    int isBinary = getIntegerInputFrom( fp, "Failed to read ASCII/Binary flag from mesh file!", _name );
    int sizeOfDouble = getIntegerInputFrom( fp, "Failed to read size of double from mesh file!", _name );
    verifyDeclaration( fp, "$EndMeshFormat", _name );
    
    // Physical names
    verifyDeclaration( fp, "$PhysicalNames", _name );
    int nPhysEnt = getIntegerInputFrom( fp, "Failed to read number of physical entities from mesh file!", _name );
    
    for ( int i = 0; i < nPhysEnt; i++ )
    {
        int dimension = getIntegerInputFrom( fp, "Failed to read physical dimension from mesh file!", _name );
        int entityNumber = getIntegerInputFrom( fp, "Failed to read physical entity number from mesh file!", _name );
        std::string physName = getStringInputFrom( fp, "Failed to read physical name from mesh file!", _name );
        
        analysisModel().domainManager().createPhysicalEntity( dimension, entityNumber, physName );
    }
    verifyDeclaration( fp, "$EndPhysicalNames", _name );
    
    // Nodes
    verifyDeclaration( fp, "$Nodes", _name );
    int nNodes = getIntegerInputFrom( fp, "Failed to read number of nodes from mesh file!", _name );
    
    RealVector coor( 3 );
    for ( int i = 0; i < nNodes; i++ )
    {
        int nodeNum = getIntegerInputFrom( fp, "Failed to read node number from mesh file!", _name );
        if ( nodeNum != i + 1 )
            throw std::runtime_error( "Node number" + std::to_string(nodeNum) + "skipped in mesh file!\nSource: " + _name );

        // Read nodal coordinates
        coor( 0 ) = getRealInputFrom( fp, "Failed reading nodal x-coor. from mesh file", _name );
        coor( 1 ) = getRealInputFrom( fp, "Failed reading nodal y-coor. from mesh file", _name );
        coor( 2 ) = getRealInputFrom( fp, "Failed reading nodal z-coor. from mesh file", _name );
        
        analysisModel().domainManager().makeNewNodeAt(coor);
        
    }
    verifyDeclaration( fp, "$EndNodes", _name );
    analysisModel().domainManager().countNodes();
    
    // Elements
    verifyDeclaration( fp, "$Elements", _name );
    int nElements = getIntegerInputFrom( fp, "Failed to read number of elements from mesh file!", _name );
    ...
    {
        // Read element number and adjust to 0-based notation
        elementNumber = getIntegerInputFrom( fp, "Failed reading element number from mesh file!", _name ) - 1;
        ...
        elementType = getIntegerInputFrom( fp, "Failed reading element type from mesh file!", _name );
        
        // Read number of tags
        int nTags = getIntegerInputFrom( fp, "Failed to read number of element tags from mesh file!", _name );

        // 1st tag is always physical entity number
        // 2nd tage is geometry entity number
        
        elementTag.assign( nTags, 0 );
        for ( int j = 0; j < nTags; j++ )
            elementTag[ j ] = getIntegerInputFrom( fp, "Failed reading element tag from mesh file!", _name );
        
        // Read node numbers
        int nElementNodes = giveNumberOfNodesForElementType( elementType );
        elementNode.assign( nElementNodes, 0 );
        
        for ( int j = 0; j < nElementNodes; j++ )
        {
            elementNode[ j ] = getIntegerInputFrom( fp, "Failed reading element nodes from mesh file!", _name );
        ...
        // Create cell
        Cell* curCell = analysisModel().domainManager().makeNewCell( elementNumber, elementType, elementTag[ 0 ] );
    ...
    verifyDeclaration( fp, "$EndElements", _name );
    
    analysisModel().domainManager().countCells();
...
// Private methods
// ----------------------------------------------------------------------------------------
int GmshReader::giveNumberOfNodesForElementType( int elType )
{
    int nNodes;
