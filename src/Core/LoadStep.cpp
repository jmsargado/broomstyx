...
{   
    ...
    int nPreprocessing = getIntegerInputFrom( fp, "Failed reading number of special pre-processing procedures for load step # " +
        std::to_string( _loadStepNum ), _name );
    
    _preProcess.assign( nPreprocessing, ProcessData() );
    for ( int i = 0; i < nPreprocessing; i++ )
    {
        _preProcess[ i ].domainTag = getStringInputFrom( fp, "Failed to read domain label for pre-processing in load step # " +
            std::to_string( _loadStepNum ), _name );
        _preProcess[ i ].directive = getStringInputFrom( fp, "Failed to read directive for pre-processing in load step # " +
            std::to_string( _loadStepNum ), _name );
    }
    ...
    val = getRealInputFrom( fp, "Failed to read start time for load step # " + std::to_string( _loadStepNum )
        + " from input file!", _name );
    _time.setStartTimeTo( val );
    
    // Time at end of load step
    verifyDeclaration( fp, "END_TIME", _name );
    val = getRealInputFrom( fp, "Failed to read end time for load step # " + std::to_string( _loadStepNum )
        + " from input file!", _name );
    _time.setEndTimeTo( val );
    
    // Initial time increment
    verifyDeclaration( fp, "INITIAL_TIME_INCREMENT", _name );
    val = getRealInputFrom( fp, "Failed to read initial time increment for load step # " + std::to_string( _loadStepNum )
        + " from input file!", _name );
    _time.setTimeIncrementTo( val );

    // Maximum number of substeps
    verifyDeclaration( fp, "MAX_SUBSTEPS", _name );
    _maxSubsteps = getIntegerInputFrom( fp, "Failed to read maximum number of substeps for load step # "
        + std::to_string( _loadStepNum ) + " from input file!", _name );
    
    // Read boundary conditions
    verifyDeclaration( fp, "BOUNDARY_CONDITIONS", _name );
    int nBC = getIntegerInputFrom( fp, "Failed to read number of boundary conditions for load step # "
        + std::to_string( _loadStepNum ) + " from input file!", _name );
    ...
    int nFC = getIntegerInputFrom( fp, "Failed to read number of field conditions for load step # "
        + std::to_string( _loadStepNum ) + " from input file!", _name );
    ...
    for ( int i = 1; i <= _nStage; i++ )
    {
        ...
        int stg = getIntegerInputFrom( fp, "Failed to read stage number in solution method input for "
            + std::string( "load step # " ) + std::to_string( _loadStepNum ), _name );
        std::string methStr = getStringInputFrom( fp, "Failed to solution method name for stage "
            + std::to_string( stg ) + " of load step # " + std::to_string( _loadStepNum ), _name );
    ...
    _writeInterval = getIntegerInputFrom( fp, "Failed to read number of output skips for load step # "
        + std::to_string( _loadStepNum ), _name );
    if ( _writeInterval < 1 )
        _writeInterval = 1;
    ...
    {
        _postProcess[ i ].domainTag = getStringInputFrom( fp, "Failed to domain label for post-processing in load step # "
            + std::to_string(_loadStepNum), _name );
        _postProcess[ i ].directive = getStringInputFrom( fp, "Failed to read directive for post-processing in load step # "
            + std::to_string(_loadStepNum), _name );
    }
}
// -----------------------------------------------------------------------------
void LoadStep::solveYourself()
{
    ...
    for ( int curStage = 1; curStage <= 4; curStage++ )
    {
...
#pragma omp parallel for
    ...
    for ( int stage = 1; stage <= _nStage; stage++ )
        this->findConstrainedDofsAtStage( stage );
    toc = std::chrono::high_resolution_clock::now();
    tictoc = toc - tic;
    std::printf( "done (time = %f sec.)\n", tictoc.count() );
    ...
    for ( int stage = 1; stage <= _nStage; stage++ )
    {
        ...
        _solutionMethod[ stage ]->imposeConstraintsAt( stage, _boundaryCondition, _time );
        _solutionMethod[ stage ]->formSparsityProfileForStage( stage );
    }
    ...
    while ( !endOfLoadStep )
    {
        ...
        for ( int curStage = 1; curStage <= _nStage; curStage++ )
        {
            ...
            while ( !substepConverged && !forceBreak )
            {
                ...
                std::printf( "\n    Stage # %d, Substep Iter. # %d", curStage, curSubstepIter );
                std::printf( "\n  --------------------------------------------\n");
                ...
                else if ( error == 1 )
                {
                    ...
                    std::printf( "(LoadStep # %d, Stage # %d, Substep # %d\n\n", _loadStepNum, curStage, curSubstep );
                    forceBreak = true;
                }
                else
                {
            ...
            if ( forceBreak )
            {
                ...
                this->performPrefinalCalculationsAtStage( curStage );

                // Finalize data (unconverged results)
                analysisModel().dofManager().finalizeDofPrimaryValuesAtStage( curStage );
                innertoc = std::chrono::high_resolution_clock::now();
                tictoc = innertoc - innertic;
                diagnostics().addUpdateTime(tictoc.count());

                analysisModel().domainManager().finalizeCellDataAt( _time, curStage );
...
// -----------------------------------------------------------------------------
void LoadStep::writeIterationDataForStage( int stg, double time, int nIter )
{
...
#pragma omp parallel for
        ...
        {
            Cell *curCell = analysisModel().domainManager().giveCell( i, dim );
            Numerics* numerics = analysisModel().domainManager().giveNumericsFor( curCell, stage );
            if ( numerics )
